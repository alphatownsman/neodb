...
class FanOutStates(StateGraph):
    ...
    def _deliver(
        ...
        Returns a terminal state when the recipient refused the activity for
        good, or None to carry on. Raises TryAgainLater when another attempt
        can still succeed.
        """
        try:
            ...
            if error.retryable:
                ...
                raise TryAgainLater() from None
            # 401/403 mean the recipient will not take this from us at all,
            # and any other 4xx is ambiguous: sending the same activity again
            # cannot resolve it, so drop it instead of retrying for days.
    ...
    def handle_new(cls, instance: "FanOut"):
        ...
        match (instance.type, instance.identity.local):
            ...
            case (FanOut.Types.post_deleted, False):
                post = instance.subject_post
                # Send it to the remote inbox; a 4xx is expected when the
                # remote processed our identity deletion earlier
                if state := cls._deliver(
                    post.author, instance, post.to_fan_out_ap(instance.type)
                ):
                    return state
