...
def test_fetch_actor_with_link_array_url(httpx_mock, config_system):
    """
    Sutty (and other distributed-press style servers) publish "url" as an array
    of Link objects covering http/ipns/hyper transports. Assigning that list
    straight to the 500-char profile_uri column raised a Postgres DataError and
    wedged the actor-refresh task; we should keep the HTML permalink instead.
    """
    identity = Identity.objects.create(
